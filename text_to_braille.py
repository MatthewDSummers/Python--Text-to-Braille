def text_to_braille(text):
    alphabet = {
        "a" : "100000", "b" : "110000", "c" : "100100", "d" : "100110",
        "e" : "100010", "f" : "110100", "g" : "110110", "h" : "110010",
        "i" : "010100", "j" : "010110", "k" : "101000", "l" : "111000",
        "m" : "101100", "n" : "101110", "o" : "101010", "p" : "111100",
        "q" : "111110", "r" : "111010", "s" : "011100", "t" : "011110",
        "u" : "101001", "v" : "111001", "w" : "010111", "x" : "101101",
        "y" : "101111", "z" : "101011", chr(32) : "000000", "uppermark" : "000001"
    }
    braille = ""
    uppermark = alphabet["uppermark"]
    for letter in text:
        if letter < chr(91) and letter > chr(64):
            letter = ord(letter) + 32
            lowercase = chr(letter)
            if lowercase in alphabet.keys():
                braille_letter = alphabet.get(lowercase)
                braille_uppercase = uppermark + braille_letter
                braille += braille_uppercase
        else:
            letter = alphabet.get(letter)
            braille += letter
    return braille
print(text_to_braille("Hello World"))